...
func PollFpmStatusMetrics(p *PhpFpmPool, pollInterval int, pollTimeout int, mustQuit chan bool, done chan bool) {
  ...
  for i := 0; i < 1; {
    ...
    cmd := exec.CommandContext(ctxWithCancel, "/usr/bin/cgi-fcgi", "-bind", "-connect", poolCpy.Endpoint)
    cmd.Env = env
    data, err = cmd.Output()
    if (err != nil) {
...
func main() {
  var (
    ...
    configDir = flag.String("config", "/etc/php5/fpm/pool.d/", "Pools conf dir")
    pollInterval = flag.Int("poll_interval", 10, "Poll interval in seconds")
    pollTimeout = flag.Int("poll_timeout", 2, "Poll timeout in seconds")
  ...
  for _, sect := range sections {
    ...
    go PollFpmStatusMetrics(&pool, *pollInterval, *pollTimeout, mustQuit, done)
